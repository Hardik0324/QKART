 ...
 for(let i=0; i<cartData.length; i++){
  for(let j=0; j<productsData.length; j++){
    if(cartData[i].productId === productsData[j]._id){
      productsData[j] = { ...productsData[j], qty:cartData[i].qty}
...
const Cart = ({
  ...
  const arri = generateCartItemsFrom(items, products)
  ...
  // const cost = getTotalCartValue(arri) 
  ...
  return (
    <>
      ...
      value={item.qty}
      handleAdd={()=>{
        handleQuantity(token, items, item._id, item.qty+1)
      }}
      handleDelete={()=>{
        handleQuantity(token, items, item._id, item.qty-1)
      }}
